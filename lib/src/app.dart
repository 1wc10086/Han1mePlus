...
String? _deepLinkRedirect(GoRouterState state) {
  ...
  if (segments.length >= 3 && segments[0] == 'videos' && segments[1] == 'hentai') {
  ...
  void initState() {
    ...
    _lifecycleListener = AppLifecycleListener(
      onExitRequested: () async {
        ...
        if (settings?.minimizeOnExit ?? true) {
...
class _StartupUpdateDownloadState extends State<_StartupUpdateDownload> {
  ...
  Future<void> _download() async {
    try {
       await UpdateInstaller(Dio()).downloadAndInstall(widget.url, (value) { if (mounted) setState(() => _progress = value); }, useMirror: widget.useMirror);
  ...
  @override
  Widget build(BuildContext context) => AlertDialog(title: Text(AppLocalizations.of(context)!.downloadingUpdate), content: _error == null ? Column(mainAxisSize: MainAxisSize.min, children: [M3ELinearProgressIndicator(value: _progress), const SizedBox(height: 12), Text(_progress == null ? AppLocalizations.of(context)!.connecting : '${(_progress! * 100).toStringAsFixed(0)}%')]) : Text(AppLocalizations.of(context)!.updateFailed(_error.toString())), actions: _error == null ? null : [TextButton(onPressed: () => Navigator.pop(context), child: Text(AppLocalizations.of(context)!.close))]);
}
...
class _AppShellState extends ConsumerState<AppShell> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final contentMediaQuery = useLiquidGlassBottomBar
        ? mediaQuery.copyWith(
            padding: mediaQuery.padding.copyWith(bottom: mediaQuery.padding.bottom + 104),
          )
        : mediaQuery;
    return Scaffold(
      ...
      bottomNavigationBar: drawerMode || useRail
          ...
          : useLiquidGlassBottomBar
               ? GlassTabBar.bottom(
                  selectedIndex: widget.navigationShell.currentIndex,
                  onTabSelected: (index) => widget.navigationShell.goBranch(index, initialLocation: index == widget.navigationShell.currentIndex),
