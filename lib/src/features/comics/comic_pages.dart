...
class ComicLibraryPage extends ConsumerWidget {
  const ComicLibraryPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = AppLocalizations.of(context)!;
    return DefaultTabController(length: 2, child: Scaffold(appBar: AppBar(title: Text(l10n.myLibrary), bottom: TabBar(tabs: [Tab(text: l10n.watchLater), Tab(text: l10n.favoriteVideos)])), body: ref.watch(comicLibraryProvider).when(loading: () => const Center(child: CircularProgressIndicator()), error: (error, _) => Text('$error'), data: (library) => TabBarView(children: [_ComicGrid(comics: library.watchLater), _ComicGrid(comics: library.favorites)]))));
  }
}
...
class _ComicCachePageState extends ConsumerState<ComicCachePage> {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    return Scaffold(
      appBar: AppBar(title: Text(l10n.cache), actions: [IconButton(tooltip: l10n.cacheCategory, onPressed: _manageCategories, icon: const Icon(Icons.folder_outlined))]),
