    ...
    return Scaffold(
      ...
      body: home.when(
        ...
        data: (feed) => RefreshIndicator(
          ...
          child: ListView(
            children: [
              _Heading(title: l10n.trendingComics),
              SizedBox(
                height: 246,
                child: CarouselView(
                  itemExtent: 160,
              ...
              _Heading(title: l10n.latestComics, action: () => context.push('/comics/browse')),
              _ComicGrid(comics: feed.latest, scrollable: false),
            ],
          ),
        ),
      ),
...
class _ComicReaderPageState extends State<ComicReaderPage> {
  ...
  Widget build(BuildContext context) {
    final comic = widget.comic;
    final long = _mode >= 4;
    if (!_ready) return Scaffold(backgroundColor: _background, body: const Center(child: CircularProgressIndicator()));
    return Scaffold(
...
class ComicLibraryPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (drawerMode) {
      ...
      return Scaffold(appBar: AppBar(leading: IconButton(onPressed: openAppDrawer, icon: const Icon(Icons.menu)), title: Text(title)), body: library.when(loading: () => const Center(child: CircularProgressIndicator()), error: (error, _) => Text('$error'), data: (value) => _ComicGrid(comics: selectedTab == 0 ? value.watchLater : value.favorites)));
    }
    return DefaultTabController(initialIndex: selectedTab, length: 2, child: Scaffold(appBar: AppBar(title: Text(l10n.myLibrary), bottom: TabBar(tabs: [Tab(text: l10n.watchLater), Tab(text: l10n.favoriteVideos)])), body: library.when(loading: () => const Center(child: CircularProgressIndicator()), error: (error, _) => Text('$error'), data: (value) => TabBarView(children: [_ComicGrid(comics: value.watchLater), _ComicGrid(comics: value.favorites)]))));
  }
}
...
class _ComicCachePageState extends ConsumerState<ComicCachePage> {
  var _category = '';
  var _categories = const <String>['默认'];
  ...
  Widget _cacheList(List<ComicCacheEntry> items, AppLocalizations l10n) {
    ...
    return Column(children: [SizedBox(height: 52, child: ListView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 12), children: categories.map((category) => Padding(padding: const EdgeInsets.only(right: 8), child: ChoiceChip(label: Text(category.isEmpty ? all : category), selected: _category == category, onSelected: (_) => setState(() => _category = category)))).toList())), Expanded(child: visible.isEmpty ? Center(child: Text(l10n.noCache)) : ListView(children: visible.map((item) => ListTile(onTap: () => _open(item), leading: CachedNetworkImage(imageUrl: item.coverUrl, width: 52, fit: BoxFit.cover), title: Text(item.title), subtitle: Text('${item.category}  ${l10n.pageCount(item.pageCount)}'), trailing: IconButton(onPressed: () => ref.read(comicCacheProvider.notifier).delete(item), icon: const Icon(Icons.delete_outline)))).toList()))]);
  }
...
class _ComicGrid extends StatelessWidget {
  const _ComicGrid({required this.comics, this.scrollable = true});
  final List<ComicCard> comics;
  final bool scrollable;

  @override
  Widget build(BuildContext context) => comics.isEmpty ? Center(child: Text(AppLocalizations.of(context)!.noComics)) : GridView.builder(shrinkWrap: !scrollable, physics: scrollable ? null : const NeverScrollableScrollPhysics(), padding: const EdgeInsets.all(16), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, childAspectRatio: .52, mainAxisSpacing: 12, crossAxisSpacing: 10), itemCount: comics.length, itemBuilder: (_, index) => ComicTile(comic: comics[index]));
}
