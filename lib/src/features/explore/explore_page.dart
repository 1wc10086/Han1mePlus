          ...
          child: CustomScrollView(
            ...
            slivers: [
              if (feed.featured != null)
                SliverToBoxAdapter(
                  child: RepaintBoundary(child: _MaxWidth(child: _FeaturedVideo(video: feed.featured!))),
                ),
              for (final section in feed.sections) _HomeSection(section: section),
            ],
          ),
        ),
      ),
...
class _HomeSectionState extends ConsumerState<_HomeSection> {
  ...
  Future<void> _prefetch() async {
    ...
    final metrics = videoCardMetrics(
      ...
      expanded: settings?.expandHomeVideoCards ?? false,
    );
