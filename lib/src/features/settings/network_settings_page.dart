...
class NetworkSettingsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(appBar: AppBar(title: Text(l10n.networkSettings)), body: ListView(children: [
      ...
      ListTile(leading: const Icon(Icons.link_outlined), title: Text(l10n.customMirrorSite), subtitle: Text(settings.mirrorActive ? settings.customMirrorSite : l10n.customMirrorSiteHint), trailing: const Icon(Icons.chevron_right), onTap: () => _showMirrorSettings(context, ref, settings, controller)),
       SwitchListTile(secondary: const Icon(Icons.dns_outlined), title: Text(l10n.useBuiltInHosts), subtitle: Text(l10n.useBuiltInHostsDescription), value: settings.useBuiltInHosts, onChanged: (value) => controller.saveChanges((current) => current.copyWith(useBuiltInHosts: value, useDoh: value ? false : current.useDoh))),
        ...
        if (Platform.isAndroid) SwitchListTile(secondary: const Icon(Icons.visibility_off_outlined), title: Text(l10n.useEch), subtitle: Text(l10n.useEchDescription), value: settings.useEch, onChanged: (value) => controller.saveChanges((current) => current.copyWith(useEch: value))),
        if (Platform.isAndroid) ListTile(leading: const Icon(Icons.receipt_long_outlined), title: Text(l10n.echLogs), subtitle: Text(l10n.echLogsDescription), trailing: const Icon(Icons.chevron_right), onTap: () => _showEchLogs(context)),
  ...
  Future<void> _showSitePicker(BuildContext context, WidgetRef ref, AppSettings settings, SettingsController controller) async {
    ...
    await controller.saveChanges((current) => current.copyWith(baseUrl: selected, videoBaseUrl: current.comicMode ? current.videoBaseUrl : selected, useCustomMirrorSite: false, customMirrorSite: ''));
