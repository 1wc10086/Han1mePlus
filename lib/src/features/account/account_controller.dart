...
final accountsProvider = FutureProvider<List<Account>>((ref) async {
  ...
  return ref.read(accountStoreProvider).readAll(settings.baseUrl);
});
...
class AccountController extends AsyncNotifier<Account?> {
  ...
  Future<Account?> build() async {
    ref.listen(settingsProvider, (previous, next) {
      if (previous?.valueOrNull?.baseUrl != null && previous?.valueOrNull?.baseUrl != next.valueOrNull?.baseUrl) ref.invalidateSelf();
    });
    final settings = await ref.watch(settingsProvider.future);
    ...
    final cloudflareCookie = await store.readCloudflareCookie(settings.baseUrl);
    if (cloudflareCookie?.isNotEmpty == true) {
      ref.read(han1meRepositoryProvider).setCloudflareCookie(cloudflareCookie!);
      if (!await http.hasCookie(settings.baseUrl, 'cf_clearance')) {
        await http.saveCookies(cloudflareCookie!, url: settings.baseUrl);
      }
    }
    final account = await store.read(settings.baseUrl);
    ...
    if (cookie != account.cookie) await store.write(settings.baseUrl, sanitizedAccount);
    ref.read(han1meRepositoryProvider).setCookie(cookie);
    await http.saveCookies(cookie, url: settings.baseUrl);
    unawaited(_refresh(sanitizedAccount));
  ...
  Future<void> saveCloudflareCookie(String cookie) async {
    ...
    await ref.read(han1meHttpClientProvider).saveCookies(cookie, url: settings.baseUrl);
    await ref.read(accountStoreProvider).writeCloudflareCookie(settings.baseUrl, cookie);
  }
  ...
  Future<void> saveCookie(String cookie) async {
    ...
    if (cloudflareCookie.isNotEmpty) {
      ref.read(han1meRepositoryProvider).setCloudflareCookie(cloudflareCookie);
      await http.saveCookies(cloudflareCookie, url: settings.baseUrl);
      await ref.read(accountStoreProvider).writeCloudflareCookie(settings.baseUrl, cloudflareCookie);
    }
    await http.saveCookies(account.cookie, url: settings.baseUrl);
    ...
    await ref.read(accountStoreProvider).write(settings.baseUrl, next);
  ...
  Future<void> activate(Account account) async {
    ...
    final settings = await ref.read(settingsProvider.future);
    final http = ref.read(han1meHttpClientProvider);
    await http.clearCookies(url: settings.baseUrl);
    ref.read(han1meRepositoryProvider).replaceCookie('');
    final cloudflareCookie = await ref.read(accountStoreProvider).readCloudflareCookie(settings.baseUrl);
    if (cloudflareCookie?.isNotEmpty == true) {
      ref.read(han1meRepositoryProvider).setCloudflareCookie(cloudflareCookie!);
      await http.saveCookies(cloudflareCookie!, url: settings.baseUrl);
    }
    ...
    await http.saveCookies(account.cookie, url: settings.baseUrl);
    await ref.read(accountStoreProvider).activate(settings.baseUrl, account.id!);
  ...
  Future<void> remove(Account account) async {
    if (account.id == null) return;
    final settings = await ref.read(settingsProvider.future);
    await ref.read(accountStoreProvider).remove(settings.baseUrl, account.id!);
    if (account.id == state.valueOrNull?.id) {
      ref.read(han1meRepositoryProvider).replaceCookie('');
      await ref.read(han1meHttpClientProvider).clearCookies(url: settings.baseUrl);
      final cloudflareCookie = await ref.read(accountStoreProvider).readCloudflareCookie(settings.baseUrl);
      if (cloudflareCookie?.isNotEmpty == true) {
        ref.read(han1meRepositoryProvider).setCloudflareCookie(cloudflareCookie!);
        await ref.read(han1meHttpClientProvider).saveCookies(cloudflareCookie!, url: settings.baseUrl);
      }
  ...
  Future<void> logout() async {
    ...
    state = const AsyncData(null);
    if (active?.id != null) await ref.read(accountStoreProvider).remove(settings.baseUrl, active!.id!);
    await ref.read(han1meHttpClientProvider).clearCookies(url: settings.baseUrl);
    if (cloudflareCookie?.isNotEmpty == true) await ref.read(han1meHttpClientProvider).saveCookies(cloudflareCookie!, url: settings.baseUrl);
