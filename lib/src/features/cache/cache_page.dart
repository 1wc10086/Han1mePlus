            ...
            Expanded(
              child: tasks.isEmpty
                  ? Center(child: Text(l10n.noCache))
                   : VideoCardGrid(
                       videos: tasks.map((task) => VideoCard(id: task.videoCode, title: task.title, coverUrl: task.coverUrl ?? '')).toList(growable: false),
    ...
    return Stack(
      ...
      children: [
        DecoratedBox(
          ...
          child: VideoCardTile(
            video: VideoCard(id: task.videoCode, title: task.title, coverUrl: task.coverUrl ?? '', duration: task.quality),
            horizontal: horizontal,
            coverImage: cover,
