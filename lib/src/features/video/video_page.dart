...
import '../search/search_page.dart' show searchableTagGroups;
...
class _VideoPageState extends ConsumerState<VideoPage> {
  final _playbackSession = VideoPlaybackSession();
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      floatingActionButton: ref.watch(videoTabProvider(widget.id)) == 1 && ref.watch(accountProvider).valueOrNull != null
           ? FloatingActionButton(onPressed: () => _writeComment(context, ref), child: const Icon(Icons.add_comment_outlined))
           : null,
      body: widget.localVideo == null ? ref.watch(videoDetailProvider(widget.id)).when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (error, stackTrace) => _VideoError(id: widget.id, error: error),
        data: (video) {
          ref.read(libraryProvider.notifier).addSubscriptionVideo(video);
          return _DetailBody(video: video, playbackSession: _playbackSession);
        },
      ) : _DetailBody(video: widget.localVideo!, playbackSession: _playbackSession),
      ),
    );
  }
  ...
  Future<void> _writeComment(BuildContext context, WidgetRef ref) async {
    ...
    final page = ref.read(commentsProvider(widget.id)).valueOrNull;
    if (text == null || text.isEmpty || page?.csrfToken == null || page?.currentUserId == null) return;
    final settings = await ref.read(settingsProvider.future);
    await ref.read(han1meRepositoryProvider).postComment(settings.baseUrl, page!.csrfToken!, page.currentUserId!, 'video', widget.id, text);
...
class _DetailBody extends ConsumerWidget {
  const _DetailBody({required this.video, required this.playbackSession});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return SafeArea(
      ...
      child: CustomScrollView(
        slivers: [
          SliverToBoxAdapter(child: VideoPlayerPanel(video: video, playbackSession: playbackSession)),
      ...
      child: Row(
        ...
        children: [
          ...
          _ActionButton(
            ...
            active: inFavorites,
            onPressed: () => account == null ? ref.read(libraryProvider.notifier).setFavorite(video, !inFavorites) : _setFavorite(ref, video.csrfToken ?? account.csrfToken, video.currentUserId ?? account.id, !inFavorites),
          ),
          _ActionButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        spacing: 8,
        runSpacing: 6,
        children: tags.map((tag) => ActionChip(label: Text('#$tag'), visualDensity: VisualDensity.compact, onPressed: () => context.push('/search', extra: _tagSearch(tag)))).toList(),
      ),
    );
  }
}
...
String _tagSearch(String tag) => searchableTagGroups.values.expand((tags) => tags).contains(tag)
    ? Uri(path: '/search', queryParameters: {'tags[]': tag}).toString()
    : Uri(path: '/search', queryParameters: {'query': tag}).toString();
