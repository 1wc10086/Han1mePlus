  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return CustomScrollView(
      slivers: [
        ...
        SliverToBoxAdapter(child: _AnimatedVideoTabBody(video: video)),
...
class _AnimatedVideoTabBody extends ConsumerWidget {
  const _AnimatedVideoTabBody({required this.video});
  final VideoDetail video;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AnimatedSwitcher(
      ...
      transitionBuilder: (child, animation) => FadeTransition(opacity: animation, child: SlideTransition(position: Tween<Offset>(begin: const Offset(.03, 0), end: Offset.zero).animate(animation), child: child)),
      child: selected == 0
          ? Column(
              key: const ValueKey('description'),
              children: [_ArtistRow(video: video), _ActionBar(video: video), _TitleBlock(video: video), _Description(video: video), _TagList(tags: video.tags), _SectionHeader(title: AppLocalizations.of(context)!.relatedVideos), _RelatedVideos(id: video.id)],
            )
          : _CommentsBody(key: const ValueKey('comments'), id: video.id),
    );
  }
}
...
class _ArtistRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final subscribed = library.artists.any((item) => item.name == video.artist);
    return Container(
      ...
      child: Row(
        children: [
          ...
          FilledButton.tonal(
            onPressed: account == null ? () => ref.read(libraryProvider.notifier).setSubscription(video, !subscribed) : null,
...
class _ActionBar extends ConsumerWidget {
      ...
      child: Row(
        ...
        children: [
          _ActionButton(
            ...
            active: inWatchLater,
            onPressed: () => account == null ? _pickLocalPlaylist(context, ref, library) : _pickPlaylist(context, ref, account.csrfToken, remote),
          ),
          _ActionButton(
  ...
  Future<void> _pickLocalPlaylist(BuildContext context, WidgetRef ref, LibraryState library) async {
    ...
    final result = await showDialog<String>(context: context, builder: (context) => AlertDialog(title: Text(AppLocalizations.of(context)!.newPlaylist), content: TextField(controller: title, autofocus: true, decoration: InputDecoration(labelText: AppLocalizations.of(context)!.name)), actions: [TextButton(onPressed: () => Navigator.pop(context), child: Text(AppLocalizations.of(context)!.cancel)), FilledButton(onPressed: () => Navigator.pop(context, title.text.trim()), child: Text(AppLocalizations.of(context)!.create))]));
    title.dispose();
  ...
  Future<void> _pickPlaylist(BuildContext context, WidgetRef ref, String? token, RemoteLibrary? library) async {
        ...
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () => Navigator.pop(context, 'saves'),
            ),
    ...
    if (selected == '__create__') {
      ...
      final result = await showDialog<(String, String)>(context: context, builder: (context) => AlertDialog(title: Text(l10n.newPlaylist), content: Column(mainAxisSize: MainAxisSize.min, children: [TextField(controller: title, decoration: InputDecoration(labelText: l10n.name)), TextField(controller: description, decoration: InputDecoration(labelText: l10n.description))]), actions: [TextButton(onPressed: () => Navigator.pop(context), child: Text(l10n.cancel)), FilledButton(onPressed: () => Navigator.pop(context, (title.text.trim(), description.text.trim())), child: Text(l10n.create))]));
      title.dispose();
      description.dispose();
...
class _CommentsBody extends ConsumerWidget {
  const _CommentsBody({super.key, required this.id});
  final String id;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (settings?.commentsEnabled != true) return Padding(padding: const EdgeInsets.all(24), child: Center(child: Text(AppLocalizations.of(context)!.commentsDisabled)));
    return ref.watch(commentsProvider(id)).when(
        loading: () => const Padding(padding: EdgeInsets.all(24), child: Center(child: CircularProgressIndicator())),
        error: (error, stackTrace) => Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(AppLocalizations.of(context)!.commentsLoadFailed), Text('$error', style: Theme.of(context).textTheme.bodySmall), FilledButton(onPressed: () => ref.invalidate(commentsProvider(id)), child: Text(AppLocalizations.of(context)!.retry))])),
        data: (page) => page.comments.isEmpty
            ? Padding(padding: const EdgeInsets.all(24), child: Center(child: Text(AppLocalizations.of(context)!.noComments)))
            : Column(children: List.generate(_visibleComments(page.comments, settings!.blockedCommentKeywords).length + 1, (index) {
                if (index == 0) return _SortControl(value: sort, onSelected: (value) => ref.read(videoCommentSortProvider(id).notifier).state = value);
                final item = _sort(_visibleComments(page.comments, settings.blockedCommentKeywords), sort)[index - 1];
                return CommentCard(comment: item, token: page.csrfToken, onChanged: () => ref.invalidate(commentsProvider(id)));
              })),
      );
  }
