        ...
        children: [
          Align(
            ...
            child: ConstrainedBox(
              ...
              child: _VideoTabsView(video: video, scrollBehavior: scrollBehavior),
            ),
          ),
          Positioned(
            left: 16,
...
class _VideoTabsView extends ConsumerStatefulWidget {
  const _VideoTabsView({required this.video, required this.scrollBehavior});
  final VideoDetail video;
...
class _VideoTabsViewState extends ConsumerState<_VideoTabsView> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(_VideoTabsView oldWidget) {
    ...
    if (oldWidget.video.id != widget.video.id) _controller.index = ref.read(videoTabProvider(widget.video.id));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        VideoPlayerPanel(video: widget.video, onBack: () => Navigator.maybePop(context)),
        Padding(
        ...
        Expanded(
          child: TabBarView(
            ...
            children: [
              M3EFloatingToolbarScrollWrapper(behavior: widget.scrollBehavior, child: _DescriptionView(video: widget.video)),
              M3EFloatingToolbarScrollWrapper(behavior: widget.scrollBehavior, child: _CommentsView(id: widget.video.id)),
            ],
          ),
        ),
      ],
...
class _CommentsView extends ConsumerStatefulWidget {
  const _CommentsView({required this.id});
  final String id;
