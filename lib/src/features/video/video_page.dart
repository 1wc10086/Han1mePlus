  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return CustomScrollView(
      slivers: [
        ...
        SliverToBoxAdapter(child: _ActionBar(video: video)),
        SliverToBoxAdapter(child: _VideoTabs(id: video.id)),
        if (ref.watch(videoTabProvider(video.id)) == 0) ...[
          SliverToBoxAdapter(child: _ArtistRow(video: video)),
          SliverToBoxAdapter(child: _TitleBlock(video: video)),
          SliverToBoxAdapter(child: _Description(video: video)),
          SliverToBoxAdapter(child: _TagList(tags: video.tags)),
          SliverToBoxAdapter(child: _SectionHeader(title: AppLocalizations.of(context)!.relatedVideos)),
          SliverToBoxAdapter(child: _RelatedVideos(id: video.id)),
        ] else
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          FilledButton.tonal(
            onPressed: () => ref.read(libraryProvider.notifier).setSubscription(video, !subscribed),
      ...
      child: Row(
        ...
        children: [
          _ActionButton(
            ...
            active: inWatchLater,
            onPressed: () => account == null ? ref.read(libraryProvider.notifier).setWatchLater(video, !inWatchLater) : _pickPlaylist(context, ref, account.csrfToken, remote),
          ),
          _ActionButton(
...
class _VideoTabs extends ConsumerWidget {
  const _VideoTabs({required this.id});
  final String id;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final selected = ref.watch(videoTabProvider(id));
    return Padding(
      padding: const EdgeInsets.fromLTRB(16, 12, 16, 4),
      child: SegmentedButton<int>(
        segments: [ButtonSegment(value: 0, icon: const Icon(Icons.article_outlined), label: Text(AppLocalizations.of(context)!.description)), ButtonSegment(value: 1, icon: const Icon(Icons.forum_outlined), label: Text(AppLocalizations.of(context)!.comments))],
        selected: {selected},
        onSelectionChanged: (value) => ref.read(videoTabProvider(id).notifier).state = value.first,
      ),
    );
  }
}
...
class _CommentsSliver extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (settings?.commentsEnabled != true) return SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(24), child: Center(child: Text(AppLocalizations.of(context)!.commentsDisabled))));
    return ref.watch(commentsProvider(id)).when(
        ...
        error: (error, stackTrace) => SliverToBoxAdapter(
          child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(AppLocalizations.of(context)!.commentsLoadFailed), Text('$error', style: Theme.of(context).textTheme.bodySmall), FilledButton(onPressed: () => ref.invalidate(commentsProvider(id)), child: Text(AppLocalizations.of(context)!.retry))])),
        ),
        data: (page) => page.comments.isEmpty
            ? SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(24), child: Center(child: Text(AppLocalizations.of(context)!.noComments))))
            : SliverList.builder(itemCount: _visibleComments(page.comments, settings!.blockedCommentKeywords).length + 1, itemBuilder: (context, index) {
