...
class _DetailBody extends ConsumerWidget {
  _DetailBody({required this.video});
  final VideoDetail video;
  final _scrollBehaviors = List.generate(2, (_) => M3EFloatingToolbarScrollBehavior.exitAlways(exitDirection: M3EFloatingToolbarExitDirection.bottom));

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final selected = ref.watch(videoTabProvider(video.id));
    final scrollBehavior = _scrollBehaviors[selected];
    return SafeArea(
      bottom: false,
      child: Stack(
        children: [
