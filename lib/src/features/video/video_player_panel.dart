...
class _VideoPlayerPanelState extends ConsumerState<VideoPlayerPanel> {
  ...
  void _saveProgress() {
    ...
    ref.read(playbackPositionProvider(widget.video.id).notifier).state = value.position.inMilliseconds;
...
class _PlayerSurfaceState extends ConsumerState<_PlayerSurface> {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    final danmaku = ref.watch(danmakuProvider(widget.videoId)).valueOrNull ?? const [];
    final settings = ref.watch(settingsProvider).valueOrNull;
    final danmakuVisible = settings?.danmakuEnabled == true && ref.watch(danmakuVisibilityProvider(widget.videoId));
    return GestureDetector(
      ...
      child: Stack(fit: StackFit.expand, children: [
        const ColoredBox(color: Colors.black),
         ...
         if (danmakuVisible) ValueListenableBuilder<VideoPlayerValue>(valueListenable: widget.controller, builder: (context, value, _) => _DanmakuOverlay(items: danmaku, positionMs: value.position.inMilliseconds, playbackSpeed: value.playbackSpeed, followsPlaybackSpeed: settings?.danmakuFollowsPlaybackSpeed == true, blockedKeywords: settings?.blockedDanmakuKeywords ?? const [])),
         ValueListenableBuilder<VideoPlayerValue>(valueListenable: widget.controller, builder: (context, value, _) => _showControls && !_locked && !(value.position == Duration.zero && !value.isPlaying) ? _Controls(controller: widget.controller, fullscreen: widget.fullscreen, onFullscreen: widget.onFullscreen, onInteraction: _restartTimer, danmakuVisible: danmakuVisible, onDanmaku: widget.fullscreen ? () => ref.read(danmakuVisibilityProvider(widget.videoId).notifier).state = !ref.read(danmakuVisibilityProvider(widget.videoId)) : null) : const SizedBox.shrink()),
...
class _DanmakuOverlay extends StatelessWidget {
  const _DanmakuOverlay({required this.items, required this.positionMs, required this.playbackSpeed, required this.followsPlaybackSpeed, required this.blockedKeywords});
  final List<Danmaku> items;
  final int positionMs;
  final double playbackSpeed;
  final bool followsPlaybackSpeed;
  ...
  Widget build(BuildContext context) {
    ...
    final visible = items.where((item) {
      final travelVideoMs = followsPlaybackSpeed ? travelMs : travelMs * (playbackSpeed == 0 ? 1 : playbackSpeed);
      final startMs = item.positionMs - leadMs;
      return !blockedKeywords.any((keyword) => item.content.toLowerCase().contains(keyword.toLowerCase())) && positionMs >= startMs && positionMs < startMs + travelVideoMs;
      ...
      child: LayoutBuilder(
        builder: (context, constraints) => Stack(
          children: visible.map((item) {
            final travelVideoMs = followsPlaybackSpeed ? travelMs : travelMs * (playbackSpeed == 0 ? 1 : playbackSpeed);
            final progress = ((positionMs - (item.positionMs - leadMs)) / travelVideoMs).clamp(0.0, 1.0).toDouble();
            final lane = item.id % 6;
            return Positioned(
              top: 16 + lane * 32.0,
              left: constraints.maxWidth * (1.0 - progress),
              child: Text(item.content, maxLines: 1, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600, shadows: [Shadow(color: Colors.black, blurRadius: 4, offset: Offset(1, 1))])),
            );
          }).toList(growable: false),
        ),
      ),
...
class _Controls extends StatelessWidget {
  const _Controls({required this.controller, required this.fullscreen, required this.onFullscreen, required this.onInteraction, required this.danmakuVisible, this.onDanmaku});
  final VideoPlayerController controller;
  final bool fullscreen;
  final Future<void> Function() onFullscreen;
  ...
  Widget build(BuildContext context) => Positioned(
    ...
    final progress = value.duration == Duration.zero ? 0.0 : value.position.inMilliseconds / value.duration.inMilliseconds;
      ...
      SizedBox(height: 28, child: Row(children: [
        ...
        Expanded(
          child: SliderTheme(
            data: SliderTheme.of(context).copyWith(trackHeight: 2, activeTrackColor: Colors.white, inactiveTrackColor: Colors.white30, thumbColor: Colors.white, overlayShape: SliderComponentShape.noOverlay),
            child: Slider(
              value: progress.clamp(0, 1).toDouble(),
              onChanged: (next) {
                controller.seekTo(Duration(milliseconds: (next * value.duration.inMilliseconds).round()));
                onInteraction();
              },
            ),
          ),
       ...
       SizedBox(height: 40, child: Row(children: [IconButton(color: Colors.white, tooltip: value.isPlaying ? l10n.pause : l10n.play, visualDensity: VisualDensity.compact, onPressed: () { value.isPlaying ? controller.pause() : controller.play(); onInteraction(); }, icon: Icon(value.isPlaying ? Icons.pause : Icons.play_arrow)), if (onDanmaku != null) IconButton(color: Colors.white, tooltip: danmakuVisible ? l10n.hideDanmaku : l10n.showDanmaku, visualDensity: VisualDensity.compact, onPressed: () { onDanmaku!(); onInteraction(); }, icon: Icon(danmakuVisible ? Icons.subtitles : Icons.subtitles_off_outlined)), const Spacer(), if (fullscreen) _SpeedMenu(controller: controller, onInteraction: onInteraction), IconButton(color: Colors.white, tooltip: fullscreen ? l10n.exitFullscreen : l10n.fullscreenPlayback, visualDensity: VisualDensity.compact, onPressed: onFullscreen, icon: Icon(fullscreen ? Icons.fullscreen_exit : Icons.fullscreen))])),
          ]);
        },
      ),
    ),
  );
  String _format(Duration duration) => duration.inHours > 0 ? '${duration.inHours}:${duration.inMinutes.remainder(60).toString().padLeft(2, '0')}:${duration.inSeconds.remainder(60).toString().padLeft(2, '0')}' : '${duration.inMinutes.toString().padLeft(2, '0')}:${duration.inSeconds.remainder(60).toString().padLeft(2, '0')}';
}
