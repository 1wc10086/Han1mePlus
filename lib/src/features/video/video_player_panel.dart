...
class _VideoPlayerPanelState extends ConsumerState<VideoPlayerPanel> {
  ...
  void _saveProgress() {
    ...
    if (value == null || !value.isInitialized || DateTime.now().difference(_lastSaved).inSeconds < 5) return;
  ...
  Widget build(BuildContext context) {
    ...
    return AspectRatio(
      ...
      child: _PlayerSurface(controller: controller, fullscreen: false, onFullscreen: _fullscreen),
    );
  }
}
...
class _PlayerSurface extends ConsumerStatefulWidget {
  const _PlayerSurface({required this.controller, required this.fullscreen, required this.onFullscreen, this.keyframes = const [], this.onKeyframes, this.onAddKeyframe});
  final VideoPlayerController controller;
...
class _Controls extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Positioned(
    ...
    final progress = value.duration == Duration.zero ? 0.0 : value.position.inMilliseconds / value.duration.inMilliseconds;
      ...
      SizedBox(height: 40, child: Row(children: [IconButton(color: Colors.white, tooltip: value.isPlaying ? l10n.pause : l10n.play, visualDensity: VisualDensity.compact, onPressed: () { value.isPlaying ? controller.pause() : controller.play(); onInteraction(); }, icon: Icon(value.isPlaying ? Icons.pause : Icons.play_arrow)), const Spacer(), if (fullscreen) _SpeedMenu(controller: controller, onInteraction: onInteraction), IconButton(color: Colors.white, tooltip: fullscreen ? l10n.exitFullscreen : l10n.fullscreenPlayback, visualDensity: VisualDensity.compact, onPressed: onFullscreen, icon: Icon(fullscreen ? Icons.fullscreen_exit : Icons.fullscreen))])),
          ]);
        },
      ),
    ),
