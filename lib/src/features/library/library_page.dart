...
class _LibraryPageState extends ConsumerState<LibraryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (drawerMode) {
      return Scaffold(
        appBar: AppBar(leading: IconButton(onPressed: openAppDrawer, icon: const Icon(Icons.menu)), title: Text(_tabTitle(l10n, widget.initialTab)), actions: widget.initialTab == 4 ? [IconButton(onPressed: () => context.push('/stats'), icon: const Icon(Icons.bar_chart_outlined))] : null),
        body: value.when(loading: () => const Center(child: CircularProgressIndicator()), error: (error, stackTrace) => Center(child: Text('$error')), data: (library) => _tabContent(library, widget.initialTab)),
      );
    }
  ...
  Widget _tabContent(LibraryState library, int index) {
    ...
    return switch (index) {
      ...
      1 => _Videos(videos: library.favorites, message: l10n.noFavoriteVideos),
  ...
  Widget build(BuildContext context) => Card(
        ...
        child: InkWell(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Expanded(child: SizedBox(width: double.infinity, child: playlist.coverUrl?.isNotEmpty == true ? Image.network(playlist.coverUrl!, fit: BoxFit.cover) : ColoredBox(color: Theme.of(context).colorScheme.surfaceContainerHighest, child: const Icon(Icons.playlist_play, size: 40)))),
