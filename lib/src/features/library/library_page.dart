        ...
        child: Scaffold(
          ...
          body: ref.watch(remoteLibraryProvider).when(
                ...
                data: (library) => TabBarView(children: [
                  ...
                  _Videos(videos: library.history, message: l10n.noWatchHistory),
                ]),
              ),
        ),
      );
...
class _RemoteSubscriptions extends StatelessWidget {
          ...
          if (artists.isNotEmpty)
            SizedBox(
              ...
              child: ListView(
                scrollDirection: Axis.horizontal,
                padding: const EdgeInsets.all(8),
                children: artists.map((artist) => Padding(padding: const EdgeInsets.only(right: 8), child: Chip(avatar: artist.avatarUrl?.isNotEmpty == true ? CircleAvatar(backgroundImage: NetworkImage(artist.avatarUrl!)) : null, label: Text(artist.name)))).toList(),
              ),
            ),
