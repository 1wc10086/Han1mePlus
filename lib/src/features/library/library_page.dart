...
class _LibraryPageState extends ConsumerState<LibraryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (account?.id != null) return const _RemoteLibrary();
    ...
    return DefaultTabController(
      ...
      child: Builder(
        builder: (context) => Scaffold(
          appBar: AppBar(title: Text(l10n.myLibrary), bottom: TabBar(isScrollable: true, tabAlignment: TabAlignment.start, tabs: _tabs(l10n))),
        ...
        if (widget.artists.isNotEmpty)
          Padding(
            ...
            child: DropdownButtonFormField<String>(
              value: selected,
              isExpanded: true,
              decoration: InputDecoration(labelText: AppLocalizations.of(context)!.subscriptions, border: const OutlineInputBorder()),
              items: widget.artists.map((artist) => DropdownMenuItem(value: artist.name, child: Row(children: [if (artist.avatarUrl?.isNotEmpty == true) CircleAvatar(radius: 12, backgroundImage: NetworkImage(artist.avatarUrl!)), if (artist.avatarUrl?.isNotEmpty == true) const SizedBox(width: 8), Expanded(child: Text(artist.name, overflow: TextOverflow.ellipsis))]))).toList(growable: false),
    ...
    return Stack(
      children: [
        widget.videos.isEmpty
            ? Center(child: Text(l10n.noWatchHistory))
            : VideoCardGrid(
                videos: widget.videos.map((video) => VideoCard(id: video.videoCode, title: video.title, coverUrl: video.coverUrl ?? '', artist: video.artistName)).toList(growable: false),
                itemBuilder: (context, index, video, horizontal) {
...
class _PlaylistVideoGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GridView.builder(
        ...
        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 10, childAspectRatio: .95),
        itemCount: videos.length,
        itemBuilder: (context, index) {
          final video = videos[index];
          final isSelected = selected.contains(video.playlistItemId);
          return Stack(fit: StackFit.expand, children: [VideoCardTile(video: VideoCard(id: video.videoCode, title: video.title, coverUrl: video.coverUrl ?? '', artist: video.artistName), horizontal: true, onTap: editing ? () => onToggle(video) : null), if (editing) Positioned(top: 4, right: 4, child: Checkbox(value: isSelected, onChanged: (_) => onToggle(video)))]);
        },
      );
}
...
class _Videos extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return VideoCardGrid(videos: videos.map((video) => VideoCard(id: video.videoCode, title: video.title, coverUrl: video.coverUrl ?? '', artist: video.artistName)).toList(growable: false));
  }
}
