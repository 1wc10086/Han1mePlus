...
class LibraryState {
  const LibraryState({this.watchLater = const [], this.favorites = const [], this.artists = const [], this.subscriptionVideos = const {}});
  ...
  Map<String, dynamic> toJson() => {'watchLater': watchLater.map((item) => item.toJson()).toList(), 'favorites': favorites.map((item) => item.toJson()).toList(), 'subscribedArtists': artists.map((item) => item.toJson()).toList(), 'subscriptionVideos': subscriptionVideos.map((key, value) => MapEntry(key, value.map((item) => item.toJson()).toList()))};
...
class LibraryController extends AsyncNotifier<LibraryState> {
  ...
  FollowingVideo _item(VideoDetail video) => FollowingVideo(videoCode: video.id, title: video.title, coverUrl: video.coverUrl, artistName: video.artist, artistAvatarUrl: video.artistAvatarUrl, genre: video.genre, addedAt: DateTime.now().millisecondsSinceEpoch);
  Future<void> _save(LibraryState value) async { state = AsyncData(value); await ref.read(libraryRepositoryProvider).save(value); }
  Future<void> setWatchLater(VideoDetail video, bool enabled) async { final current = state.value ?? const LibraryState(); final items = current.watchLater.where((item) => item.videoCode != video.id).toList(); if (enabled) items.insert(0, _item(video)); await _save(LibraryState(watchLater: items, favorites: current.favorites, artists: current.artists, subscriptionVideos: current.subscriptionVideos)); }
  Future<void> setFavorite(VideoDetail video, bool enabled) async { final current = state.value ?? const LibraryState(); final items = current.favorites.where((item) => item.videoCode != video.id).toList(); if (enabled) items.insert(0, _item(video)); await _save(LibraryState(watchLater: current.watchLater, favorites: items, artists: current.artists, subscriptionVideos: current.subscriptionVideos)); }
  Future<void> setSubscription(VideoDetail video, bool enabled) async { final name = video.artist?.trim() ?? ''; if (name.isEmpty) return; final current = state.value ?? const LibraryState(); final id = _artistId(name); final artists = current.artists.where((artist) => artist.id != id).toList(); final videos = Map<String, List<FollowingVideo>>.from(current.subscriptionVideos); if (enabled) { artists.insert(0, SubscribedArtist(id: id, name: name, avatarUrl: video.artistAvatarUrl, genre: video.genre, addedAt: DateTime.now().millisecondsSinceEpoch)); videos[id] = [_item(video), ...?videos[id]?.where((item) => item.videoCode != video.id)]; } else { videos.remove(id); } await _save(LibraryState(watchLater: current.watchLater, favorites: current.favorites, artists: artists, subscriptionVideos: videos)); }
  Future<void> addSubscriptionVideo(VideoDetail video) async { final current = state.value ?? const LibraryState(); final name = video.artist?.trim() ?? ''; final id = _artistId(name); if (name.isEmpty || !current.artists.any((artist) => artist.id == id)) return; final videos = Map<String, List<FollowingVideo>>.from(current.subscriptionVideos); videos[id] = [_item(video), ...?videos[id]?.where((item) => item.videoCode != video.id)]; await _save(LibraryState(watchLater: current.watchLater, favorites: current.favorites, artists: current.artists, subscriptionVideos: videos)); }
