...
class DownloadController extends AsyncNotifier<DownloadState> {
  ...
  Future<DownloadState> build() async {
    ref.listen<AsyncValue<AppSettings>>(settingsProvider, (previous, next) => _schedule());
    final root = await getExternalStorageDirectory() ?? await getApplicationDocumentsDirectory();
    _root = Directory('${root.path}/Download');
    ...
    try {
      ...
      return DownloadState(groups: loaded.groups, tasks: loaded.tasks.map((task) => task.status == DownloadStatus.downloading ? task.copyWith(status: DownloadStatus.queued) : task).toList());
    } catch (_) {
  ...
  Future<void> deleteTasks(Set<String> ids) async {
    ...
    for (final task in current.tasks.where((task) => ids.contains(task.id))) {
      await Directory('${_root.path}/${task.videoCode}').delete(recursive: true);
  ...
  Future<void> create(VideoDetail detail, VideoSource source, String groupId) async {
    ...
    final task = DownloadTask(id: detail.id, videoCode: detail.id, title: detail.title, coverUrl: detail.coverUrl, groupId: groupId, quality: source.quality, status: DownloadStatus.queued, progress: 0, downloadedBytes: 0, totalBytes: 0, createdAt: now, updatedAt: now);
    final current = state.value ?? const DownloadState();
