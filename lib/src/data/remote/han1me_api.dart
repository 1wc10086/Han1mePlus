...
class Han1meApi {
  ...
  static const _headers = {
    'User-Agent': 'Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125 Mobile Safari/537.36',
  ...
  Future<RemoteLibrary> library(String baseUrl, String userId) async {
    final pages = await Future.wait([_document('$baseUrl/user/$userId/saves', skipCache: true), _document('$baseUrl/user/$userId/likes', skipCache: true), _document('$baseUrl/user/$userId/histories', skipCache: true), _document('$baseUrl/user/$userId/playlists', skipCache: true)]);
    final playlistPage = pages[3];
    ...
    return RemoteLibrary(watchLater: _libraryVideos(baseUrl, pages[0]), favorites: _libraryVideos(baseUrl, pages[1]), history: _libraryVideos(baseUrl, pages[2]), playlists: playlists, csrfToken: playlistPage.querySelector('input[name="_token"]')?.attributes['value'] ?? pages[0].querySelector('input[name="_token"]')?.attributes['value']);
  }
  ...
  Future<CommentPage> comments(String baseUrl, String videoId, {String type = 'video'}) async {
    ...
    final response = await _dio.get<String>(
      '$base/loadComment',
      queryParameters: {'type': type, 'id': videoId},
      options: Options(
        ...
        headers: {
          ...
          'Referer': '$base/watch?v=$videoId',
          if (_cookie != null) 'Cookie': _cookie,
        },
      ),
    );
    if (_isCloudflare(response)) throw CloudflareChallengeException('$base/watch?v=$videoId');
    if (response.statusCode == null || response.statusCode! >= 400) {
       throw DioException(requestOptions: response.requestOptions, message: 'Comment request failed: HTTP ${response.statusCode}');
    }
  ...
  Future<CommentPage> replies(String baseUrl, String commentId) async {
    final response = await _dio.get<String>('$baseUrl/loadReplies', queryParameters: {'id': commentId}, options: Options(responseType: ResponseType.plain, headers: {..._headers, if (_cookie != null) 'Cookie': _cookie}));
  ...
  Future<dom.Document> _document(String url, {String? referer, bool skipCache = false}) async {
    final response = await _dio.get<String>(url, options: Options(
      validateStatus: (_) => true,
      extra: {'cacheDuration': _cacheDuration(url), if (skipCache) 'skipCache': true},
      headers: {
        ..._headers,
    ...
    if (_isCloudflare(response)) throw CloudflareChallengeException(url);
    if (response.statusCode == null || response.statusCode! >= 400) {
       throw DioException(requestOptions: response.requestOptions, message: 'Request failed: HTTP ${response.statusCode}');
    }
    return html_parser.parse(response.data ?? '');
  }

  Future<void> _form(String url, Map<String, String> data, String token) async {
    final response = await _dio.post<String>(url, data: FormData.fromMap(data), options: Options(validateStatus: (_) => true, headers: {..._headers, 'X-CSRF-TOKEN': token, if (_cookie != null) 'Cookie': _cookie}));
    if (response.statusCode == null || (response.statusCode! >= 400 && response.statusCode != 302)) throw DioException(requestOptions: response.requestOptions, message: 'Request failed: HTTP ${response.statusCode}');
  }
  ...
  Duration _cacheDuration(String url) => switch (Uri.parse(url).path) {
        '/' => const Duration(minutes: 5),
        '/search' => const Duration(minutes: 3),
        final path when path.startsWith('/previews/') => const Duration(hours: 6),
        '/watch' => const Duration(minutes: 10),
        _ => const Duration(minutes: 3),
      };

  bool _isCloudflare(Response<String> response) => response.statusCode == 403 && (response.headers.value('cf-mitigated') == 'challenge' || (response.data ?? '').contains('cf-chl-'));
