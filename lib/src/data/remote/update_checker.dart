...
class UpdateChecker {
  ...
  final Dio _dio;
  Future<UpdateInfo?> check({String currentVersion = '1.0.2'}) async {
    try {
      ...
      final asset = (data['assets'] as List? ?? const []).whereType<Map>().cast<Map>().where((item) => '${item['name'] ?? ''}'.toLowerCase().endsWith('.apk')).firstOrNull;
      final downloadUrl = '${asset?['browser_download_url'] ?? ''}'.trim();
      if (tag.isEmpty || !_newer(tag, currentVersion)) return null;
      return UpdateInfo(tagName: tag, htmlUrl: '${data['html_url'] ?? ''}', body: '${data['body'] ?? ''}', createdAt: '${data['created_at'] ?? ''}', downloadUrl: downloadUrl, prerelease: data['prerelease'] == true);
    } catch (_) { return null; }
  }
