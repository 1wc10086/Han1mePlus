...
class UpdateChecker {
  ...
  Future<UpdateInfo?> check({String? currentVersion}) async {
    try {
      ...
      final preferredAssets = assets.where((item) => '${item['name'] ?? ''}'.toLowerCase().endsWith(preferredSuffix));
      final asset = Platform.isWindows
          ? preferredAssets.where((item) { final name = '${item['name'] ?? ''}'.toLowerCase(); return name.contains('setup') || name.contains('installer'); }).firstOrNull ?? preferredAssets.firstOrNull
          : preferredAssets.firstOrNull ?? (fallbackSuffix.isEmpty ? null : assets.where((item) => '${item['name'] ?? ''}'.toLowerCase().endsWith(fallbackSuffix)).firstOrNull);
