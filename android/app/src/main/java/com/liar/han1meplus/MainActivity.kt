            ...
            try {
                ...
                httpClient.newCall(request.build()).execute().use {
                    val payload = mapOf(
                        ...
                        "body" to (it.body?.let { body ->
                            val charset = call.argument<String>("responseCharset")?.let(Charset::forName)
                            if (charset == null) body.string() else body.bytes().toString(charset)
                        } ?: ""),
